...
public class EntityToSiriMapper {
    ...
    public static PtSituationElement mapToPtSituationElement(PtSituationElementEntity dto) {
        ...
        ptSituationElement.setSource(
                SiriObjectFactory.createSituationSourceStructure(
                        dto.getSource().getSourceType()
                )
        );

        ptSituationElement.setProgress(
                WorkflowStatusEnumeration.fromValue(
                        Objects.requireNonNull(dto.getProgress())
                )
        );

        ptSituationElement.getValidityPeriods().add(
                SiriObjectFactory.createValidityPeriod(
                        Objects.requireNonNull(
                                dto.getValidityPeriod().getStartTime()
                        ),
        ...
        ptSituationElement.setReportType(
                ReportTypeEnumeration.fromValue(
                        Objects.requireNonNull(
                                dto.getReportType()
                        )
                )
        ...
        ptSituationElement.getSummaries().add(
        ...
        Optional.ofNullable(
                dto.getDescription()
        ).ifPresent(text -> ptSituationElement.getDescriptions().add(
                SiriObjectFactory.createDefaultedTextStructure(
        ...
        Optional.ofNullable(
                dto.getAdvice()
        ).ifPresent(text -> ptSituationElement.getAdvices().add(
                SiriObjectFactory.createDefaultedTextStructure(
                        text.getAttributes().getXmlLang(),
                        text.getText()
                )
        ...
        ptSituationElement.setAffects(
                SiriObjectFactory.createAffectsScopeStructure(
                        Optional.ofNullable(
                                dto.getAffects().getNetworks()
                        ).map(networks -> {
                            var lineRef = networks.getAffectedNetwork().getAffectedLine().getLineRef();

                            var stopPointRefs = Optional.ofNullable(
                                    networks.getAffectedNetwork().getAffectedLine().getRoutes()
                            ).map(routes -> routes.getAffectedRoute().getStopPoints().getAffectedStopPoint().stream().map(
                                    affectedStopPoint -> SiriObjectFactory.createAffectedStopPointStructure(affectedStopPoint.getStopPointRef())
                                ).toList()
                            ).orElse(null);
                        ...
                        Optional.ofNullable(
                                dto.getAffects().getStopPoints()
                        ).map(stopPoints ->
                                stopPoints.getAffectedStopPoint().stream().map(affectedStopPoint ->
                                        SiriObjectFactory.createAffectedStopPointStructure(
                                                affectedStopPoint.getStopPointRef()))
                                        .toList())
                                .orElse(null),
                        Optional.ofNullable(
                                dto.getAffects().getVehicleJourneys()
                        ).map(vehicleJourneys -> SiriObjectFactory.createAffectedVehicleJourneyStructure(
                                vehicleJourneys.getAffectedVehicleJourney().getFramedVehicleJourneyRef().getDataFrameRef(),
                                vehicleJourneys.getAffectedVehicleJourney().getFramedVehicleJourneyRef().getDatedVehicleJourneyRef(),
                                Optional.ofNullable(
                                        vehicleJourneys
                                                .getAffectedVehicleJourney()
                                                .getRoute()
                                ).map(route -> route.getStopPoints().getAffectedStopPoint().stream().map(affectedStopPoint -> SiriObjectFactory.createAffectedStopPointStructure(affectedStopPoint.getStopPointRef())).toList())
                                .orElse(null)
                        )).orElse(null)
        ));
